...
export default function App() {
  ...
  if (appMode === 'portal') {
    return (
      <div className="min-h-screen bg-[#0B0F19] flex flex-col items-center justify-center p-4 selection:bg-[#3B82F6]/30 font-sans relative overflow-hidden">
        ...
        <div className="text-center mb-12 relative z-10">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-white tracking-widest uppercase mb-4">Medifli Platform</h1>
          <p className="text-slate-400 text-lg max-w-md mx-auto">Select your portal to continue to the application.</p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-4xl relative z-10">
          <button 
            onClick={() => setAppMode('student-signup')}
            className="group bg-[#131B2F] hover:bg-[#1e293b] border border-[#1e293b] hover:border-[#3B82F6] p-8 rounded-3xl transition-all duration-300 text-left flex flex-col items-start cursor-pointer relative overflow-hidden shadow-xl"
          >
            ...
            <p className="text-slate-400">Access your dashboard, practice modes, and deep diagnostics.</p>
          </button>

          <button 
            onClick={() => setAppMode('admin')}
            className="group bg-[#131B2F] hover:bg-[#1e293b] border border-[#1e293b] hover:border-emerald-500 p-8 rounded-3xl transition-all duration-300 text-left flex flex-col items-start cursor-pointer shadow-xl"
          >
            ...
            <p className="text-slate-400">Manage curriculum, upload question banks, and monitor platform health.</p>
          </button>
        </div>
  ...
  return (
    <div className="flex h-screen bg-[#020617] text-slate-100 font-sans overflow-hidden selection:bg-blue-500/30 selection:text-blue-100">
      {/* Mobile Header */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[#0B0F19] border-b border-slate-800 z-50 flex items-center justify-between px-4 shadow-sm">
        <div className="flex items-center space-x-2 text-blue-500">
          <BrainCircuit className="w-6 h-6" />
          <span className="font-bold text-xl tracking-tight text-white">MEDIFLI</span>
        </div>
        <button 
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="md:hidden fixed inset-0 bg-[#0B0F19]/80 backdrop-blur-sm z-40"
      ...
      {/* Sidebar */}
      <div className={`fixed inset-y-0 left-0 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition duration-300 ease-in-out z-50 md:z-0`}>
        <Sidebar 
      ...
      {/* Main Content */}
      <main className="flex-1 h-full overflow-y-auto pt-16 md:pt-0 bg-[#0B0F19] relative">
        <DustBackground />
