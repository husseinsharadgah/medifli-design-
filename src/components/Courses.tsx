...
export function Courses({ setCurrentView }: { setCurrentView: (view: string) => void }) {
  ...
  return (
    <div className="max-w-[1600px] mx-auto space-y-10 pb-12 text-slate-100 font-sans">
      
      {/* Header Area */}
      <header className="flex flex-col xl:flex-row xl:justify-between xl:items-end pb-4 border-b border-[#1e293b] relative z-50">
        <div>
        ...
        <div className="mt-6 xl:mt-0 flex flex-col sm:flex-row items-stretch sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
          ...
          <div className="relative">
            ...
            {/* Dropdown Menu */}
            {isDropdownOpen && (
              <div className="absolute top-full lg:right-0 left-0 lg:left-auto mt-2 w-[320px] bg-[#0B0F19] border border-[#1e293b] rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] z-40 overflow-hidden outline outline-1 outline-white/5">
                <div className="py-2">
          ...
          {/* Stats Box */}
          <div className="flex items-center space-x-3 text-sm text-slate-400 font-medium bg-[#131B2F] px-4 py-2.5 rounded-xl border border-[#1e293b]">
            <BookOpen className="w-4 h-4 text-[#3B82F6]" />
            <span>{filteredActive.length + filteredCompleted.length} Courses</span>
            <span className="w-px h-4 bg-[#1e293b]"></span>
      ...
      {filteredActive.length > 0 && (
        <section>
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h3 className="text-xl font-bold text-white tracking-wide uppercase">Active Curriculum</h3>
          </div>
              ...
              return (
                <div key={course.id} className="group relative overflow-hidden rounded-2xl border border-[#1e293b] bg-[#131B2F] cursor-pointer h-[400px] shadow-md hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 flex flex-col">
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-center relative z-10 bg-[#131B2F]">
                    <h3 className="text-xl font-bold text-white mb-2 leading-tight line-clamp-2 pr-8">{course.title}</h3>
                  ...
                  <div className="absolute inset-0 bg-[#0B0F19]/95 backdrop-blur-xl p-6 flex flex-col translate-y-[101%] group-hover:translate-y-0 transition-transform duration-300 ease-in-out z-30">
                    ...
                    <h4 className="text-lg font-black text-white mb-4 border-b border-[#1e293b]/80 pb-2 flex-shrink-0">Course Progress</h4>

                    {/* Meta / Instructor Info */}
                    <div className="flex items-center space-x-3 mb-6 flex-shrink-0">
                       <div className={`w-10 h-10 rounded-xl ${course.bg} flex items-center justify-center`}>
                    ...
                    <div className="flex-1 mt-4">
                      <div className="bg-[#131B2F] border border-[#1e293b] rounded-xl p-4">
                        ...
                        <div className="w-full bg-[#0B0F19] rounded-full h-2.5 border border-[#1e293b] overflow-hidden">
                          <div 
      ...
      {filteredCompleted.length > 0 && (
        <section className="pt-4">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h3 className="text-xl font-bold text-white tracking-wide uppercase">Completed Modules</h3>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-8">
            {filteredCompleted.map((course) => (
              <div key={course.id} className="group relative overflow-hidden rounded-2xl border border-[#1e293b] bg-[#131B2F] cursor-pointer h-[400px] shadow-md hover:shadow-xl hover:shadow-emerald-500/10 transition-all duration-300 flex flex-col">
                ...
                <div className="p-6 flex-1 flex flex-col justify-center relative z-10 bg-[#131B2F]/90 backdrop-blur-sm">
                  <div className="flex justify-between items-start mb-3">
                    <h3 className="text-xl font-bold text-white leading-tight line-clamp-2 pr-2">{course.title}</h3>
                  </div>
                  <div className="flex items-center justify-between text-sm mt-auto">
                     <span className="italic text-slate-400">{course.instructor}</span>
                ...
                <div className="absolute inset-0 bg-[#0B0F19]/95 backdrop-blur-xl p-6 flex flex-col translate-y-[101%] group-hover:translate-y-0 transition-transform duration-300 ease-in-out z-30">
                  ...
                  <h4 className="text-lg font-black text-white mb-4 border-b border-emerald-500/20 pb-2 flex-shrink-0 text-emerald-400 flex items-center space-x-2">
                  ...
                  <div className="flex-1 mt-4">
                     <div className="bg-[#131B2F] border border-[#1e293b] rounded-xl p-4 text-center">
                        ...
                        <p className="text-lg font-bold text-white tracking-widest">{course.completedDate}</p>
                     </div>
                  </div>
      ...
      {filteredActive.length === 0 && filteredCompleted.length === 0 && (
         <div className="flex flex-col items-center justify-center py-20 px-4 text-center rounded-2xl border border-dashed border-[#1e293b] bg-[#131B2F]/30 mt-8">
