    ...
    return (
      <div className="max-w-[1000px] mx-auto pt-4 pb-12 font-sans h-full flex flex-col animation-fade-in">
        <button 
          ...
          className="flex items-center space-x-2 text-slate-400 hover:text-white transition-colors mb-8 self-start group"
        >
        ...
        {/* Header */}
        <div className="flex items-center space-x-4 mb-10 pb-6 border-b border-[#1e293b]">
          ...
          <div>
            <div className="flex items-center space-x-3 mb-1">
              <h2 className="text-3xl font-black text-white tracking-widest uppercase">Drill Tuning</h2>
            ...
            <p className="text-slate-400 font-medium">Configuring session for <span className={`${selectedModeData.color}`}>{selectedModeData.title}</span></p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          ...
          {/* Target Selection */}
          <div className="space-y-6 bg-[#131B2F] border border-[#1e293b] p-6 rounded-3xl">
            <h3 className="text-white font-bold tracking-wide uppercase flex items-center space-x-2 mb-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-[#0B0F19] border border-[#1e293b] text-white rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 transition-colors appearance-none cursor-pointer"
                >
                  ...
                  <button
                    ...
                    className={`py-2 px-1 rounded-lg text-xs font-bold tracking-wide transition-all border ${
                      difficulty === lvl 
                        ...
                        : 'bg-[#0B0F19] text-slate-400 border-[#1e293b] hover:border-slate-600'
                    }`}
                  >
                    {lvl}
                  </button>
          ...
          <div className="space-y-6 bg-[#131B2F] border border-[#1e293b] p-6 rounded-3xl md:col-span-2">
            <h3 className="text-white font-bold tracking-wide uppercase flex items-center space-x-2 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {/* Tutor Mode Toggle */}
              <div className="flex items-center justify-between bg-[#0B0F19] p-4 rounded-xl border border-[#1e293b]">
                <div>
                  <h4 className="text-sm font-bold text-white tracking-wide">Tutor Mode</h4>
                  <p className="text-xs text-slate-400 mt-1">Show correct answers & explanations immediately after each question</p>
                </div>
                <button 
        ...
        <div className="mb-10 bg-amber-500/10 border border-amber-500/30 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between shadow-[0_0_30px_rgba(245,158,11,0.05)]">
           <div className="flex items-center space-x-4 mb-4 md:mb-0">
              ...
              <div>
                 ...
                 <p className="text-white text-lg">You have a drill in progress. (Question {frozenState.currentIndex + 1} / 10)</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 flex-1">
        {QUIZ_MODES.map((mode) => (
          <button
            ...
            className={`group flex flex-col items-start p-8 rounded-3xl border border-[#1e293b] bg-[#131B2F] cursor-pointer transition-all duration-300 ${mode.hoverBorder} hover:shadow-xl hover:-translate-y-1 relative overflow-hidden text-left`}
          >
            ...
            <div className="relative z-10 flex-1">
              <h3 className="text-2xl font-bold text-white mb-3 tracking-wide">{mode.title}</h3>
              <p className="text-slate-400 leading-relaxed">
                {mode.description}
              </p>
            </div>

            <div className="mt-8 flex items-center space-x-2 text-slate-500 group-hover:text-white transition-colors relative z-10 font-bold uppercase tracking-widest text-sm">
